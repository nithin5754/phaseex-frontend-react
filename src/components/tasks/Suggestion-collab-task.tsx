...
import { selectTaskCollabSpace, setSearchTaskQuery } from "@/app/redux/slice/taskSlice"
import { SendAddCollabTaskType, useAddCollaboratorToTaskMutation } from "@/app/redux/api/taskapi"
...
interface Props {
  ...
  const SuggestionTaskCollab = ({workspaceId,folderId,listId,taskId}:Props) => {
          ...
          <CommandGroup heading="suggestions " className="bg-slate-900" >
            {getTaskCollabSuggestion && getTaskCollabSuggestion.length > 0? (
              getTaskCollabSuggestion.map((query) => (
                ...
                <UserCircle2Icon size={14} className="mr-4" onClick={() => dispatch(setSearchTaskQuery(query.fullName))}/>

const AuthSideImg = () => {
  return (
    <div className="lg:flex hidden w-1/2 bg-gradient-to-tr ">
      <div className="h-[40rem] w-full rounded-md bg-neutral-950 relative flex flex-col items-center justify-around antialiased">
        <h1 className="absolute top-4 left-2 z-50 text-lg md:text-2xl  bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600  text-center font-sans font-bold">
          PHASEEX
        </h1>
 

        {/* <div className="max-w-2xl mx-auto p-4">
        <h1 className="relative z-10 text-lg md:text-7xl bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600 text-center font-sans font-bold">
          One Website <br />
          <span className="block text-lg md:text-5xl mt-6">
            Many Roles
          </span>
        </h1>
          <p className="text-neutral-500 max-w-md  mx-auto my-2 text-2xl text-center relative z-10">
      
          </p>
        </div> */}


<div className="max-w-2xl mx-auto p-4">
<h1 className="relative z-10 text-lg md:text-7xl bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600 text-center font-sans font-bold">
          One Website <br />
          <span className="block text-lg md:text-6xl mt-6 mb-4">
            Many Roles
          </span>
        </h1>
        <p></p>
        <p className="text-neutral-500 max-w-lg mx-auto my-2 text-sm text-center relative z-10">
        An all-in-one app to replace to multiple others
        </p>
    
      </div>

      
      </div>
    </div>
  );
};
export default AuthSideImg;

...
import { addAllGroupToSlice, selectAllPrompt } from "@/app/redux/slice/geminiSlice";
...
const navigate=useNavigate()
  ...
  const {data:getAllGroup}=useGetAllGroupQuery(undefined,  {
   ...
   useEffect(()=>{
     if(getAllGroup!==undefined&&getAllGroup.length>0){
      let uploadData: {
        groupId: string;
        title: string;
        userId: string;
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <h4 className="mb-4 text-sm font-medium leading-none text-center">chat group</h4>
        <Separator/>
        {
      getAllGroup!==undefined&&getAllGroup.length>0&&getAllGroup.map((grp:GetAllGroupType)=>{

        return(

     <div className="flex items-center justify-center">
     
     <SidebarLink key={grp.id} link={{
            label:grp.title,
